...
`openscad4.py` geometry library, with automatic use of the real OpenSCAD
...
You already have the packages `openscad4.py` needs (`numpy`, `scipy`,
...
`openscad4.py` function name in Python mode, or the language's keywords in
...
every `openscad4.py` function and this app's own `union()`/`show()`/etc
...
`openscad4.py`'s 600+ functions plus the DSL's built-in primitives/
...
technically visible because `openscad4.py` opens with `from numpy
...
**Python** — plain Python that calls your `openscad4.py` functions
...
One name is intentionally shadowed: `openscad4.py` already has its own
...
`openscad4.py`** (`cube`, `sphere`, `cylinder`, `circle`, `square`,
...
through either CSG pipeline — they come straight from `openscad4.py` and
...
backend/
  openscad4.py            <- your library, unmodified
  scad_lang.py            <- tokenizer + parser for the OpenSCAD-style language
  scad_eval.py            <- walks the parsed script, calls openscad4.py, applies transforms
